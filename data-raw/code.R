...
linhas_set <- paste("SET c comarcas / c1*c",as.character(n_comarcas)," /;",sep="")
linhas_set %<>% paste("\n", "ALIAS(c,v)", "\n", sep="")
...
linhas_parameter %<>% paste("\t/ c1 ", perc_processos[1], "\n", sep = "")
for(ii in 2:(n_comarcas - 1)) {
  linhas_parameter %<>% paste("\t  c", ii, " ", perc_processos[ii], "\n", sep="")
}
linhas_parameter %<>% paste("\t  c", n_comarcas, " ", perc_processos[n_comarcas], " /;\n", sep="")


dist_comarcas <- data_varas[1:n_comarcas, 3:(n_comarcas+2)] %>% as.matrix %>%
...
for(jj in 0:(n_comarcas %/% 10)) {
  ...
  cols <- indices %>% sapply(function(idx) paste("\tc", idx, sep="")) %>% paste(collapse=" ")
  linha_cols %<>% paste(cols, sep="")
  linhas_parameter_2 %<>% paste(linha_cols, "\n", sep="")
  for(ii in 1:n_comarcas)
  {
    linha_distancias <- paste("c",ii," ",sep="")
    dist_string <- dist_comarcas[ii,indices] %>% sapply(function(dist) paste("\t", dist, sep="")) %>%
                                                 paste(collapse=" ")
    linha_distancias %<>% paste(dist_string,"\n", sep="")
    linhas_parameter_2 %<>% paste(linha_distancias, sep="")
  }
}
...
igdx("/opt/gams/gams24.6_linux_x64_64_sfx/")
gdxrrw::gams('ALOC_VARAS.GMS')
